...
{
    const longMessages = messages(0, 5, floor => `第${floor}层${'很长的剧情内容'.repeat(220)}`);
    const adaptive = niMemoryBuildNextBatch(longMessages, niMemoryCreateEmptyStore(), 10, {
        tokenLimit: 3000,
    });
    assert.ok(adaptive);
    assert.ok(adaptive.entries.length < 6);
    assert.ok(adaptive.estimatedTokens <= 3000 || adaptive.entries.length === 1);
}

{
    const batch = niMemoryBuildNextBatch(messages(0, 9), niMemoryCreateEmptyStore(), 10);
    const densePayload = {
        summary: '十层剧情的详细导航概览。',
        events: Array.from({ length: 6 }, (_, index) => ({
            kind: index === 0 ? 'dialogue' : 'action',
            text: `原子事件${index}`,
            quote: index === 0 ? '钥匙只能交给你' : '',
            actors: ['林默'],
            locations: [],
            objects: index === 0 ? ['黑曜密钥'] : [],
            tags: ['细节'],
            importance: 2,
            certainty: 'explicit',
            evidence_floors: [index],
        })),
        state_changes: [],
        knowledge_changes: [],
        open_threads: [],
        skipped_floors: [6, 7, 8, 9].map(floor => ({ floor, reason: '没有新增世界内信息' })),
    };
    assert.equal(niMemoryValidateExtractPayload(densePayload, batch), densePayload);
    assert.throws(() => niMemoryValidateExtractPayload({
        ...densePayload,
        skipped_floors: densePayload.skipped_floors.slice(1),
    }, batch), /未覆盖楼层：6/);
    const leaf = niMemoryNormalizeLeafPayload(densePayload, batch);
    assert.equal(leaf.events[0].kind, 'dialogue');
    assert.equal(leaf.events[0].quote, '钥匙只能交给你');
    assert.equal(leaf.skippedFloors.length, 4);
    assert.equal(leaf.sourceMessageCount, 10);
}
...

{
    const events = Array.from({ length: 20 }, (_, index) => ({
        ...
        certainty: 'explicit',
        evidenceFloors: [index],
    }));
    const records = niMemoryBuildCompactionRecords([{ ...leafFor(0, 9), events }]);
    assert.equal(records[0].events.length, 12);
    assert.ok(records[0].events.every(event => event.importance >= 3));
}
...

{
    ...
    store = niMemoryApplyLeaf(store, leafFor(10, 19, {
        summary: '林默与艾琳在北境港口会合。',
        events: [{ kind: 'dialogue', text: '林默把黑曜密钥交给艾琳，并要求她不要告诉议会', quote: '别让议会知道钥匙在你手里', actors: ['林默', '艾琳'], locations: ['北境港口'], objects: ['黑曜密钥'], tags: ['归还', '保密'], importance: 5, certainty: 'explicit', evidence_floors: [18] }],
    }));
    ...
    assert.match(injection, /相关事件细节/);
    assert.match(injection, /别让议会知道钥匙在你手里/);
    const details = niMemoryRecallEventDetails(store, '议会为什么不知道钥匙', { documents: recalled, limit: 4 });
    assert.equal(details[0].event.quote, '别让议会知道钥匙在你手里');
...

{
    ...
    const elements = {
        ...
        '#ni-memory-latest': { innerHTML: '' },
        '#ni-memory-update': { disabled: false, innerHTML: '' },
    };
    const settings = {
        pluginEnabled: true,
        ...
        branchMemoryPrompt: '旧版自定义提示词：请处理以下聊天，只输出 JSON。\n{CURRENT_CHAT}',
    };
    let extractCall = null;
    const controller = createBranchMemoryController({
        getSettings: () => settings,
        defaultSettings: settings,
        getContext: () => ({ chat, saveChat: async () => { saves++; } }),
        callApiSeq: async (messages, options) => {
            extractCall = { messages, options };
            return JSON.stringify({
                ...
                knowledge_changes: [],
                open_threads: [],
                skipped_floors: Array.from({ length: 9 }, (_, floor) => ({ floor, reason: '没有新增世界内信息' })),
            });
        },
        query: selector => elements[selector] || null,
        saveSettings: () => {},
        confirm: () => true,
        alert: () => {},
        logger: { warn: () => {} },
    ...
    assert.match(elements['#ni-memory-latest'].innerHTML, /展开详细账本/);
    assert.match(elements['#ni-memory-latest'].innerHTML, /林默抵达北境港口/);
}
