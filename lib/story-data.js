...
function niNormalizeCharacterFactText(value) {
    return String(value || '')
        .replace(/\s+/g, '')
        .replace(/[，。；;：:、"'“”‘’（）()【】\[\]]/g, '')
        .toLowerCase();
...
export function niMergeCharacterFactText(currentValue, incomingValue, maxLength = 2400) {
    const current = String(currentValue || '').trim();
    const incoming = String(incomingValue || '').trim();
    if (!current) return incoming.slice(0, maxLength);
    if (!incoming) return current.slice(0, maxLength);

    const currentNorm = niNormalizeCharacterFactText(current);
    const incomingNorm = niNormalizeCharacterFactText(incoming);
    if (currentNorm === incomingNorm || currentNorm.includes(incomingNorm)) {
    ...
    [...current.split(/\s*(?:；|;|\n)\s*/), ...incoming.split(/\s*(?:；|;|\n)\s*/)]
        ...
        .filter(Boolean)
        .forEach(item => {
            const normalized = niNormalizeCharacterFactText(item);
            if (!normalized) return;
            const coveredBy = merged.findIndex(existing =>
                niNormalizeCharacterFactText(existing).includes(normalized));
            if (coveredBy >= 0) return;
            const covers = merged.findIndex(existing =>
                normalized.includes(niNormalizeCharacterFactText(existing)));
            if (covers >= 0) merged[covers] = item;
...
export function mergeCharacters(state, incoming, chunkIndex) {
    ...
    for (const character of incoming) {
        ...
        if (!existing) {
            ...
            const mergedIdentity = niMergeCharacterFactText(existing.identity, character.identity || character.bio || '', 1600);
            const mergedAppearance = niMergeCharacterFactText(existing.appearance, character.appearance || '', 1000);
            const mergedPersonality = niMergeCharacterFactText(existing.personality, character.personality || '', 2400);
            const mergedRelations = niMergeCharacterFactText(existing.relations, character.relations || '', 2400);
            if (incomingChunkIdx != null && (existingChunkIdx == null || incomingChunkIdx < existingChunkIdx)) {
                existing._firstChunkIdx = incomingChunkIdx;
                existing._characterId = 'char:' + incomingChunkIdx + ':' + niHashShort(existing.name);
            }
            if ((!existing.role || existing.role === '其他') && character.role && character.role !== '其他') {
                existing.role = character.role;
            }
            if (!existing.gender && character.gender) existing.gender = character.gender;
            existing.identity = mergedIdentity;
            existing.appearance = mergedAppearance;
            existing.personality = mergedPersonality;
...
export const NI_CHARACTER_PROFILE_RESPONSE_BUDGETS = Object.freeze([4096, 7000, 10000, 12000]);

export const NI_CHARACTER_PROFILE_CONTEXT_BUDGETS = Object.freeze([
    Object.freeze({ baseProfile: 2400, previousProfile: 1400, historySummary: 4500, recentChat: 8000, novelCtx: 1800 }),
    Object.freeze({ baseProfile: 2200, previousProfile: 1200, historySummary: 4000, recentChat: 6500, novelCtx: 1000 }),
    Object.freeze({ baseProfile: 1800, previousProfile: 1000, historySummary: 3400, recentChat: 5200, novelCtx: 600 }),
    Object.freeze({ baseProfile: 1400, previousProfile: 800, historySummary: 2800, recentChat: 4200, novelCtx: 0 }),
]);

export function niIsCharacterProfileLengthError(error) {
    const message = String(error?.message || error || '').toLowerCase();
    return message.includes('ai 返回被长度截断')
        || message.includes('finish_reason=length')
        || message.includes('finish reason: length');
}

export function niAdvanceCharacterProfileRetry(state = {}, error = null) {
    const currentBudgetIndex = Math.max(0, Math.min(
        NI_CHARACTER_PROFILE_RESPONSE_BUDGETS.length - 1,
        parseInt(state.responseBudgetIndex, 10) || 0,
    ));
    const currentCompactLevel = Math.max(0, Math.min(
        NI_CHARACTER_PROFILE_CONTEXT_BUDGETS.length - 1,
        parseInt(state.compactLevel, 10) || 0,
    ));
    const lengthLimited = niIsCharacterProfileLengthError(error);
    const responseBudgetIndex = lengthLimited
        ? Math.min(currentBudgetIndex + 1, NI_CHARACTER_PROFILE_RESPONSE_BUDGETS.length - 1)
        : currentBudgetIndex;
    const compactLevel = Math.min(
        currentCompactLevel + 1,
        NI_CHARACTER_PROFILE_CONTEXT_BUDGETS.length - 1,
    );
    return {
        lengthLimited,
        responseBudgetIndex,
        responseLength: NI_CHARACTER_PROFILE_RESPONSE_BUDGETS[responseBudgetIndex],
        compactLevel,
        retryReason: lengthLimited ? 'length' : 'format',
    };
}

function niClipCharacterProfileText(value, maxChars, keep = 'start') {
    const text = String(value || '').trim();
    const limit = Math.max(0, parseInt(maxChars, 10) || 0);
    if (!text || !limit) return '';
    if (text.length <= limit) return text;

    const marker = '…（已按重试预算省略）…';
    if (limit <= marker.length + 4) return text.slice(0, limit);
    const available = limit - marker.length - 1;
    if (keep === 'end') return `${marker}\n${text.slice(-available)}`;
    if (keep === 'both') {
        const headLength = Math.ceil(available * 0.58);
        const tailLength = available - headLength;
        return `${text.slice(0, headLength)}\n${marker}${text.slice(-tailLength)}`;
    }
    return `${text.slice(0, available)}\n${marker}`;
}

/**
 * 为实时人设生成设置固定的分区输入预算。重试级别越高，越优先保留近期对话、
 * 分支历史和上次人设，逐步压缩最低优先级的原著材料。
 */
export function niCompactCharacterProfileContext(context = {}, compactLevel = 0) {
    const level = Math.max(0, Math.min(
        NI_CHARACTER_PROFILE_CONTEXT_BUDGETS.length - 1,
        parseInt(compactLevel, 10) || 0,
    ));
    const budget = NI_CHARACTER_PROFILE_CONTEXT_BUDGETS[level];
    return {
        ...context,
        baseProfile: niClipCharacterProfileText(context.baseProfile, budget.baseProfile, 'start'),
        previousProfile: niClipCharacterProfileText(context.previousProfile, budget.previousProfile, 'start'),
        historySummary: niClipCharacterProfileText(context.historySummary, budget.historySummary, 'both'),
        recentChat: niClipCharacterProfileText(context.recentChat, budget.recentChat, 'end'),
        novelCtx: niClipCharacterProfileText(context.novelCtx, budget.novelCtx, 'end'),
        _compactLevel: level,
    };
}
...
export function niSelectCharacterEvidenceMessages(messages, terms, {
    ...
    const rawMessages = (Array.isArray(messages) ? messages : [])
        .filter(message => {
            ...
            return !!name && name !== 'system' && name !== 'sillytavern system';
        })
        .slice(-messageLimit)
...
function niCharacterHistoryFloorLabel(item) {
    const floors = Array.isArray(item?.evidenceFloors)
        ? item.evidenceFloors.filter(value => Number.isFinite(Number(value))).slice(0, 4)
        : [];
    if (floors.length) return `第${floors.join('、')}楼`;
    const start = Number(item?.startFloor);
    const end = Number(item?.endFloor);
    if (Number.isFinite(start) && Number.isFinite(end)) return start === end ? `第${start}楼` : `第${start}-${end}楼`;
    const updated = Number(item?.updatedFloor ?? item?.sourceFloor ?? item?.createdFloor);
    return Number.isFinite(updated) ? `第${updated}楼` : '';
}

function niCharacterHistoryTextMatches(value, terms) {
    if (Array.isArray(value)) return value.some(item => niCharacterHistoryTextMatches(item, terms));
    if (value && typeof value === 'object') {
        return niCharacterHistoryTextMatches(Object.values(value), terms);
    }
    return niCharacterEvidenceHasTarget(String(value || ''), terms);
}

/**
 * 从聊天级分支记忆与偏差总结中提取目标角色相关证据。
 * 只读取已经压缩的结构化记录，并用分层/条数/字符预算避免长跑团线性膨胀。
 */
export function niBuildCharacterHistoryContext({
    branchMemory = null,
    deviation = null,
    terms = [],
    textLimit = 4500,
} = {}) {
    const targetTerms = [...new Set((Array.isArray(terms) ? terms : [])
        .map(term => String(term || '').trim())
        .filter(term => term.length >= 2))];
    const maxChars = Math.max(800, Math.min(10000, parseInt(textLimit, 10) || 4500));
    if (!targetTerms.length) return { text: '', hasTargetEvidence: false, sourceCount: 0 };

    const matches = value => niCharacterHistoryTextMatches(value, targetTerms);
    const memory = branchMemory && typeof branchMemory === 'object' ? branchMemory : {};
    const sections = [];
    const seen = new Set();
    let sourceCount = 0;
    let usedChars = 0;

    const addSection = (title, candidates, limit) => {
        const lines = [];
        for (const candidate of (Array.isArray(candidates) ? candidates : []).slice(0, limit)) {
            const line = String(candidate || '').replace(/\s+/g, ' ').trim();
            if (!line) continue;
            const dedupeKey = niNormalizeCharacterEvidenceText(line.replace(/^\[[^\]]+\]\s*/, ''));
            if (!dedupeKey || seen.has(dedupeKey)) continue;
            const prefixCost = lines.length ? 1 : title.length + 3;
            const available = maxChars - usedChars - prefixCost;
            if (available < 80) break;
            const clipped = line.slice(0, available);
            lines.push(clipped);
            seen.add(dedupeKey);
            sourceCount++;
            usedChars += prefixCost + clipped.length;
    ...
    const byFreshness = (a, b) =>
        Number(b?.updatedFloor ?? b?.endFloor ?? b?.sourceFloor ?? b?.createdFloor ?? 0)
        - Number(a?.updatedFloor ?? a?.endFloor ?? a?.sourceFloor ?? a?.createdFloor ?? 0)
        || Number(b?.importance || 0) - Number(a?.importance || 0);
    const evidenceSuffix = item => {
        const label = niCharacterHistoryFloorLabel(item);
        const uncertain = item?.certainty === 'uncertain' ? '；未确认' : '';
        return label || uncertain ? `（${[label, uncertain.replace(/^；/, '')].filter(Boolean).join('；')}）` : '';
    };

    const currentStates = Object.values(memory.currentState || {})
        .filter(item => matches(item?.entity))
        .sort(byFreshness)
        .map(item => `- ${item.entity || '目标角色'}／${item.field || '状态'}：${item.value || ''}${evidenceSuffix(item)}`);
    addSection('当前有效状态', currentStates, 10);

    const knowledgeLabels = { knows: '知道', believes: '相信', suspects: '怀疑', does_not_know: '尚不知道' };
    const knowledge = Object.values(memory.knowledge || {})
        .filter(item => matches(item?.holder) || matches(item?.fact))
        .sort(byFreshness)
        .map(item => `- ${item.holder || '角色'}${knowledgeLabels[item.status] || '认为'}：${item.fact || ''}${evidenceSuffix(item)}`);
    addSection('角色认知与他人视角', knowledge, 8);

    const threads = Object.values(memory.openThreads || {})
        .filter(item => matches(item?.participants) || matches(item?.text))
        .sort(byFreshness)
        .map(item => `- ${item.text || ''}${evidenceSuffix(item)}`);
    addSection('未决关系与线索', threads, 6);

    const relevantLeaves = (Array.isArray(memory.leaves) ? memory.leaves : [])
        .filter(leaf => matches(leaf?.entities) || matches(leaf?.summary) || matches(leaf?.events))
        .sort(byFreshness)
        .slice(0, 10);
    const leafLines = relevantLeaves.map(leaf => {
        const events = (Array.isArray(leaf?.events) ? leaf.events : [])
            .filter(event => matches(event?.actors) || matches(event?.text))
            .sort((a, b) => Number(b?.importance || 0) - Number(a?.importance || 0))
            .slice(0, 3)
            .map(event => `${event.certainty === 'uncertain' ? '【未确认】' : ''}${event.text}`);
        const body = events.length ? events.join('；') : String(leaf?.summary || '');
        return `- [${niCharacterHistoryFloorLabel(leaf) || '历史记录'}] ${body}`;
    });
    addSection('历史行为证据', leafLines, 10);

    const oldestLeafFloor = relevantLeaves.length
        ? Math.min(...relevantLeaves.map(leaf => Number(leaf?.startFloor || 0)))
        : Infinity;
    const compactRecords = [
        ...(Array.isArray(memory.arcs) ? memory.arcs : []),
        ...(Array.isArray(memory.episodes) ? memory.episodes : []),
    ]
        .filter(record => Number(record?.endFloor || 0) < oldestLeafFloor)
        .filter(record => matches(record?.entities) || matches(record?.summary) || matches(record?.keyEvents))
        .sort(byFreshness)
        .map(record => `- [${niCharacterHistoryFloorLabel(record) || '更早记录'}] ${record.summary || (record.keyEvents || []).join('；')}`);
    addSection('更早阶段概括', compactRecords, 5);

    // 新长期记忆是角色连续性的主来源。只有它完全没有该角色证据时，
    // 才回退旧版偏差事实，避免两套总结同时进入同一次人设生成。
    if (sourceCount === 0) {
        const deviationFacts = (Array.isArray(deviation?.facts) ? deviation.facts : [])
            .filter(fact => fact?.status !== 'retired' && matches(fact?.text))
            .sort(byFreshness)
            .map(fact => `- ${fact.text || ''}${evidenceSuffix(fact)}`);
        addSection('旧版偏差事实（回退）', deviationFacts, 6);
    }

    return {
        text: sections.join('\n\n').slice(0, maxChars),
        hasTargetEvidence: sourceCount > 0,
        sourceCount,
    };
}
...
export function niNormalizeDeviationFact(value, index = 0) {
    ...
    const rawImportance = Number(objectValue?.importance);
    const importance = Number.isFinite(rawImportance)
        ? Math.max(1, Math.min(5, Math.round(rawImportance)))
        : null;
    const irreversible = typeof objectValue?.irreversible === 'boolean'
        ? objectValue.irreversible
        : null;
    return {
        id,
        text,
...
export function niBuildDeviationFactsContext(source) {
    return niActiveDeviationFacts(source).map(fact => ({
        ...
        irreversible: fact.irreversible === true,
...
export function niReconcileDeviationFacts(existing, incoming, {
    ...
    incomingFacts.forEach((incomingFact) => {
        ...
        if (old) {
            ...
            nextFacts.push({
                ...
                importance: incomingFact.importance ?? old.importance ?? null,
                irreversible: incomingFact.irreversible ?? old.irreversible ?? null,
                updatedFloor,
    ...
    const oldIds = new Set(oldFacts.map(fact => niDevCleanText(fact.id)).filter(Boolean));
    const removedIdentifiers = removedValues
        .map(value => niDevCleanText(value && typeof value === 'object' ? value.id ?? value.factId ?? value.fact_id : value))
        .filter(Boolean);
    const removedKeys = new Set(
        removedIdentifiers.filter(value => oldIds.has(value) || value.startsWith('fact:')),
    );
    const removedTextKeys = new Set(
        removedValues
            .map(value => niDevCleanText(value && typeof value === 'object' ? value.text ?? value.fact : value))
            .filter(value => value && !removedKeys.has(value))
...
const NI_DEV_INJECTION_HARD_FACT_RE = /死亡|已死|杀死|永久|不可逆|身份暴露|真实身份|关系断裂|决裂|背叛|阵营|能力(?:丧失|失去|获得|改变)|世界规则|封锁|流放|结婚|怀孕|失忆|毁灭|摧毁|重大秘密|关键承诺/;

function niDevInjectionTerms(value) {
    const text = String(value || '').normalize('NFKC').toLocaleLowerCase();
    const terms = [];
    text.match(/[a-z0-9][a-z0-9_.:-]{1,31}/g)?.forEach(term => terms.push(term));
    text.match(/[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff]+/g)?.forEach(run => {
        if (run.length === 1) terms.push(run);
        for (let index = 0; index < run.length - 1; index++) terms.push(run.slice(index, index + 2));
    });
    const stop = new Set(['当前', '剧情', '角色', '已经', '现在', '他们', '我们', '这个', '一个']);
    return terms.filter(term => !stop.has(term)).slice(0, 1600);
}

function niDevConstraintLines(value) {
    return String(value || '')
        .replace(/\s+(\d+[.、]\s*)/g, '\n$1')
        .split(/\r?\n/)
        .map(line => line.trim())
...
export function niIsLikelyGarbledDeviationText(value) {
    const text = String(value ?? '');
    if (!text.trim()) return false;
    if (/[\u0000-\u0008\u000B\u000C\u000E-\u001F\uFFFD]/.test(text)) return true;
    if (/(?:锟斤拷|烫烫烫|屯屯屯|ï¿½|(?:Ã.|Â.|â€){2,})/i.test(text)) return true;
    return /[^\p{L}\p{N}\s，。！？；：、“”‘’（）【】《》…—·,.!?;:'"()[\]{}<>/\\_+=-]{10,}/u.test(text);
}

export function niParseContinuityConstraintText(value) {
    const result = { hard: [], scene: [], invalidated: [], legacy: [], structured: false };
    let section = 'legacy';
    for (const rawLine of niDevConstraintLines(value)) {
        const heading = rawLine.match(/^【([^】]+)】$/)?.[1] || '';
        if (heading) {
            if (/不可回滚|硬约束|永久约束/.test(heading)) section = 'hard';
            else if (/当前场景|场景约束|临时状态/.test(heading)) section = 'scene';
            else if (/失效.*原著|原著.*失效/.test(heading)) section = 'invalidated';
            else section = 'legacy';
            if (section !== 'legacy') result.structured = true;
            continue;
        }
        const line = rawLine.replace(/^[-*•]\s*/, '').replace(/^\d+[.、]\s*/, '').trim();
        if (line && !niIsLikelyGarbledDeviationText(line)) result[section].push(line);
    }
    return result;
}

function niSelectLegacyContinuityConstraints(lines, query, limit = 6) {
    const queryTerms = new Set(niDevInjectionTerms(query));
    return (Array.isArray(lines) ? lines : [])
        .map((line, index) => {
            const terms = new Set(niDevInjectionTerms(line));
            const matchHits = [...terms].filter(term => queryTerms.has(term)).length;
            const hard = NI_DEV_INJECTION_HARD_FACT_RE.test(line) || /不得|严禁|不可|不能.*恢复|禁止.*拉回原著/.test(line);
            return { line, index, matchHits, hard, score: matchHits * 4 + (hard ? 20 : 0) };
        })
        .filter(item => item.hard || item.matchHits > 0)
...
/**
 * 偏差面板继续保存完整档案；实际写作注入只发送少量不可逆/相关事实与当前约束。
 * “仍保留的原著事实”由原著召回层负责，不在这里重复发送。
 */
export function niBuildDeviationInjectionGuide(source = {}, {
    query = '',
    maxFacts = 8,
    maxChars = 1800,
    hasBranchMemory = false,
} = {}) {
    const sections = niNormalizeDeviationSections(source);
    const facts = niActiveDeviationFacts(sections)
        .filter(fact => !niIsLikelyGarbledDeviationText(fact.text));
    const queryTerms = new Set(niDevInjectionTerms(query));
    const latestFloor = facts.reduce((max, fact) => Math.max(max, niDevFactDisplayFloor(fact) ?? 0), 0);
    const scored = facts.map((fact, index) => {
        const factTerms = new Set(niDevInjectionTerms(fact.text));
        const matchHits = [...factTerms].filter(term => queryTerms.has(term)).length;
        const hard = fact.irreversible === true || NI_DEV_INJECTION_HARD_FACT_RE.test(fact.text);
        const importance = fact.importance ?? (hard ? 4 : (['relationship', 'world_rule'].includes(fact.kind) ? 3 : 2));
        const floor = niDevFactDisplayFloor(fact) ?? 0;
        const recency = latestFloor > 0 ? 1 / (1 + Math.max(0, latestFloor - floor) / 200) : 0;
        return {
            fact,
            index,
            matchHits,
            hard,
            score: matchHits * 4 + importance * 1.4 + recency + (fact.irreversible === true ? 30 : (hard ? 5 : 0)),
        };
    }).sort((a, b) => b.score - a.score || b.index - a.index);

    const limit = Math.max(1, Math.min(12, parseInt(maxFacts, 10) || 8));
    const selected = [];
    const selectedIds = new Set();
    const take = item => {
        if (!item || selected.length >= limit || selectedIds.has(item.fact.id)) return;
        selectedIds.add(item.fact.id);
        selected.push(item);
    };
    const hardLimit = hasBranchMemory ? Math.min(3, limit) : Math.min(4, limit);
    scored.filter(item => item.hard).slice(0, hardLimit).forEach(take);
    scored.filter(item => item.matchHits > 0).forEach(take);
    if (!hasBranchMemory) scored.forEach(take);
    selected.sort((a, b) => (niDevFactDisplayFloor(a.fact) ?? Infinity) - (niDevFactDisplayFloor(b.fact) ?? Infinity));

    const charLimit = Math.max(500, Math.min(5000, parseInt(maxChars, 10) || 1800));
    const parsedConstraint = niParseContinuityConstraintText(sections.currentConstraint);
    const legacyConstraints = niSelectLegacyContinuityConstraints(parsedConstraint.legacy, query, hasBranchMemory ? 4 : 6);
    const parts = [];
    let used = 0;
    const appendBlock = (heading, lines) => {
        const list = (Array.isArray(lines) ? lines : []).map(line => String(line || '').trim()).filter(Boolean);
        if (!list.length || used >= charLimit) return;
        const prefix = `【${heading}】\n`;
        const available = charLimit - used - prefix.length - (parts.length ? 2 : 0);
        if (available < 60) return;
        const block = `${prefix}${niDevClipMiddle(list.map(line => `- ${line}`).join('\n'), available)}`;
        parts.push(block);
        used += block.length + 2;
    };
    if (selected.length) {
        ...
        for (const item of selected) {
            ...
            const floor = niDevFactDisplayFloor(fact);
            const tags = [
                fact.irreversible === true ? '不可逆' : '',
                fact.kind && fact.kind !== 'fact' ? fact.kind : '',
                floor != null ? `第${floor}楼` : '',
            ].filter(Boolean).join('·');
            const line = `- ${tags ? `【${tags}】` : ''}${fact.text}`;
            lines.push(line);
        }
        appendBlock('连续性防火墙·不可回滚事实', lines.map(line => line.replace(/^[-*•]\s*/, '')));
    }
    appendBlock('连续性防火墙·不可回滚约束', parsedConstraint.hard);
    appendBlock('连续性防火墙·当前场景', parsedConstraint.scene);
    appendBlock('连续性防火墙·已失效原著前提', parsedConstraint.invalidated);
    appendBlock('连续性防火墙·旧记录筛选结果', legacyConstraints);
    return parts.join('\n\n').slice(0, charLimit).trim();
}
...
export function createStoryController(deps = {}) {
    ...
    const NI_CHAR_AI_PROFILE_RETRIES = NI_CHARACTER_PROFILE_RESPONSE_BUDGETS.length - 1;
    ...
    function niBuildCharAiProfileContext(c, idx) {
        ...
        const chatRoot = Array.isArray(ctx?.chat) ? ctx.chat[0] : null;
        const history = niBuildCharacterHistoryContext({
            branchMemory: chatRoot?.ni_branch_memory,
            deviation: chatRoot?.ni_dev,
            terms: evidenceTerms,
        });
        const previous = niGetCharAiProfile(idx);
        const previousProfile = previous ? [
            previous.identity ? `身份、目标与处境：${previous.identity}` : '',
            previous.appearance ? `持续外貌或状态：${previous.appearance}` : '',
            previous.personality ? `性格模型：${previous.personality}` : '',
            previous.relations ? `重要关系：${previous.relations}` : '',
        ].filter(Boolean).join('\n') : '';
        ...
        const novelCtx = allNodes
            ...
            .slice(-8)
            .map(p => `[阶段${p.stageIdx || p._stageIdx || '未知'}·${p.title}] ${p.body}`)
            .join('\n')
            .slice(-3000);
        ...
        return {
            ...
            previousProfile,
            novelCtx,
            hasTargetEvidence: evidence.hasTargetEvidence || history.hasTargetEvidence,
            hasRecentEvidence: evidence.hasTargetEvidence,
            hasHistoryEvidence: history.hasTargetEvidence,
            isUserSubTarget,
        };
    }
    
    function niBuildCharAiProfilePrompt(c, charCtx, { retryReason = '' } = {}) {
        const userSubRule = charCtx?.isUserSubTarget
            ? '目标角色就是当前用户代入的原著角色；只有当近期对话或分支历史明确以 <user> 或目标角色可靠别名描写其状态时，才可作为目标角色证据。'
            : '目标角色不是 <user>。任何 <user>、用户、玩家、我、你 的身份、外貌、性格和关系都不得写入目标角色。';
        const retryRule = retryReason === 'length'
            ? '\n【本次为长度截断后的重试】上次回复在 JSON 完成前触及输出上限。不要输出思考过程、解释、前言或代码块；直接给出更精炼的单行 JSON，优先保证闭合与六个字段完整。\n'
            : (retryReason === 'format'
                ? '\n【本次为格式修复重试】上次回复无法解析。不要输出思考过程、解释、前言或代码块；只输出一个闭合的单行 JSON。\n'
                : '');
    
        return `你是角色连续性编辑器。请只为目标角色【${c.name}】生成可直接用于演绎、简短但不脸谱化的当前分支人设。
    ${retryRule}
    ...
    【上次分支人设（连续性底稿；未被新证据推翻的内容应保留）】
    ${charCtx?.previousProfile || '（无）'}

    【目标角色相关分支历史总结】
    ${charCtx?.historySummary || '（无）'}

    【近期直接对话证据（发生更晚，冲突时优先）】
    ${charCtx?.recentChat || '（无）'}
    
    【目标角色相关原著节点（最低优先级，只作固定背景参考）】
    ${charCtx?.novelCtx || '（无）'}
    
    证据顺序：近期直接对话 > 分支历史总结 > 上次分支人设 > 原著稳定底稿。新证据只更新它实际改变的部分；没有被推翻的既有信息继续保留。

    总结规则：
    - 输出的是“当前分支状态层”，不要机械复述原著。近期对话或分支历史至少有一处目标角色证据时 appeared=true；两者都没有时 appeared=false，四个人设字段均为空字符串
    - 禁止将发生在其他角色、<user>、叙事视角人物或被称为“我/你”的对象身上的事件推断或转移到【${c.name}】身上
    - 其他角色对【${c.name}】的评价只能写成“他人视角”，不能直接当作其客观性格；原著不能单独证明当前分支状态
    - 若全部分支证据都没有出现【${c.name}】或可靠别名（${charCtx?.targetTerms || c.name}），不得仅凭代词、泛称或原著生成
    - identity 可写当前身份、阵营、公开状态、目标与处境；appearance 只写当前仍持续有效且影响场景的外貌、伤病或装束变化
    - personality 必须压缩成一幅“性格调色盘”，让演绎模型先理解人物，再按情境调用，而不是每轮展示全部特征：
      1) 调色盘：底色写最稳定的价值取向/核心需要；主色写1-2个反复出现的决策倾向；副色写较少出现、能与主色形成张力的倾向；衍生写“哪些颜色在何种触发下组合成什么反应”
      2) 二次解释：解释行为背后的目标、顾虑或防御机制。只能采用证据支持的深层原因；证据不足就保留条件性，不得擅自补童年创伤、爱恨或病理诊断
      3) 三面性：优先写【${c.name}】面对不同具体角色时的不同状态；只有证据存在才写实际人名，不足时宁可少写，不得套用“对亲近者/对陌生人/对敌人”的空模板
    - 稳定人格需有重复行为、明确旁白或未被推翻的既有人设支撑；单次反应只能写成“在该情境下的倾向”。允许矛盾，但要交代触发条件；避免“永远、必然、绝不会”以及无依据的“外冷内热、腹黑、病娇”等标签
    - relations 写具体对象、当前关系、变化原因和未解决张力；与 personality 的三面性互补，不重复堆字
    - 严格控制字数，按下面结构输出，不输出任何其他文字：
    {"target":"${c.name}","appeared":true,"identity":"当前身份、目标与处境60字内","appearance":"持续有效的外貌或状态40字内或空字符串","personality":"调色盘：底色=…；主色=…；副色=…。二次解释：…。三面性：对A…；对B…。衍生：触发→反应。总计260字内","relations":"当前重要关系、变化依据与张力120字内或空字符串"}
    ...
    - 是否只记录【${c.name}】本人的信息，并正确区分直接证据、历史总结和他人视角
    - 是否把单次反应误写成稳定人格，或把复杂人物压成一个标签
    - 是否没有 Markdown、代码块或结构外文本`;
    }
    ...
    async function niGenerateCharAiProfileWithRetry(i, charCtx, onRetry = null, { signal = null, noEvidenceMode = 'skip' } = {}) {
        ...
        if (!charCtx?.hasTargetEvidence) {
            ...
            throw niCharAiSkipError(`近期对话与分支历史总结中都没有「${c.name}」或可靠别名的证据`);
        }
    
        let lastErr = null;
        let responseBudgetIndex = 0;
        let compactLevel = 0;
        let retryReason = '';
        for (let attempt = 0; attempt <= NI_CHAR_AI_PROFILE_RETRIES; attempt++) {
            if (signal?.aborted) throw new Error('请求已中止（超时或用户操作）');
            try {
                const requestContext = niCompactCharacterProfileContext(charCtx, compactLevel);
                const responseLength = NI_CHARACTER_PROFILE_RESPONSE_BUDGETS[responseBudgetIndex];
                const raw = await callApiSeq([{
                    role: 'user',
                    content: niBuildCharAiProfilePrompt(c, requestContext, { retryReason }),
                }], { responseLength, signal });
                ...
                if (attempt < NI_CHAR_AI_PROFILE_RETRIES) {
                    const retryState = niAdvanceCharacterProfileRetry({ responseBudgetIndex, compactLevel }, e);
                    responseBudgetIndex = retryState.responseBudgetIndex;
                    compactLevel = retryState.compactLevel;
                    retryReason = retryState.retryReason;
                    onRetry?.(attempt + 1, e, retryState);
    ...
    async function niGenCharsManual(silent = false, skipIndices = null, {
        ...
        await niRunGenerationPool(enabledIndices, workerCount, async (i, ei) => {
            ...
            try {
                ...
                const profile = await niGenerateCharAiProfileWithRetry(i, charCtx, (retryNo, err, retryMeta) => {
                    const retryDetail = retryMeta?.lengthLimited
                        ? `，长度截断，输出预算升至 ${retryMeta.responseLength}`
                        : '，修复输出格式';
                    if (note) note.textContent = `并发 ${workerCount} · ${c.name}（重试 ${retryNo}/${NI_CHAR_AI_PROFILE_RETRIES}${retryDetail}）`;
        ...
        if (note) {
            if (cancelled) {
                ...
                if (cleared) parts.push(`清空 ${cleared} 位无分支证据`);
                if (skipped) parts.push(`跳过 ${skipped} 位无分支证据`);
        ...
        if (btn) {
            ...
            btn.title = '调用 AI 更新角色人设（原著底稿+分支历史总结+近期对话）';
        }
    ...
    async function niGenOneCharManual(i) {
        ...
        try {
            ...
            if (niIsCharAiSkipError(e)) {
                alert(`近期对话与分支历史总结中都没有「${c.name}」或可靠别名的证据，已保留现有 AI 人设。`);
            } else {
