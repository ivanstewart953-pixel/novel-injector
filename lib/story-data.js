...
export function niBuildDeviationInjectionGuide(source = {}, {
    ...
    const constraintLimit = Math.max(240, Math.floor(charLimit * 0.56));
    const constraint = sections.currentConstraint
        ? niDevClipMiddle(sections.currentConstraint, constraintLimit)
        : '';
    const parts = [];
    let used = 0;
    if (selected.length) {
        const heading = '【关键分支偏差】\n';
        const lines = [];
        for (const item of selected) {
            ...
            const projected = heading.length + lines.join('\n').length + line.length + 1;
            if (projected > charLimit - constraint.length - 20) break;
            lines.push(line);
        }
        if (lines.length) {
    ...
    if (constraint && used < charLimit) {
        const heading = '【当前偏差约束】\n';
        const available = Math.max(80, charLimit - used - heading.length - 2);
        parts.push(`${heading}${niDevClipMiddle(constraint, available)}`);
    }
