...
export function niBuildStageRuntimeIndex(state = {}, { resolveCharFirstStage = null } = {}) {
    ...
    mainPlots.forEach((plot, index) => {
        ...
        primaryEntries.push({ plot, stageIdx });
    ...
    const pickNearestPrimaryStage = (subPlot, candidates) => {
        const usable = (candidates || []).filter(entry => entry.stageIdx != null);
        if (!usable.length) return null;
        const subOrder = niPlotStoryOrder(subPlot, 0);
        usable.sort((a, b) => {
            const aOrder = niPlotStoryOrder(a.plot, 0);
            const bOrder = niPlotStoryOrder(b.plot, 0);
            return Math.abs(aOrder - subOrder) - Math.abs(bOrder - subOrder) || aOrder - bOrder;
        });
        return usable[0].stageIdx;
    };
    ...
    subPlots.forEach(plot => {
        ...
        if (stageIdx == null) {
            ...
            if (title) {
                stageIdx = pickNearestPrimaryStage(plot, primaryEntries.filter(({ plot: parent }) =>
                    Array.isArray(parent?.branch_links) && parent.branch_links.includes(title)
                ));
            }
        }
        ...
        if (stageIdx == null) {
            const chunkIdx = niPlotChunkIdx(plot, -1);
            stageIdx = pickNearestPrimaryStage(plot, primaryEntries.filter(({ plot: parent }) =>
                niPlotChunkIdx(parent, -2) === chunkIdx
            ));
        }
