...
export function createStoryController(deps = {}) {
    ...
    const NI_CHAR_AI_PROFILE_RESPONSE_LENGTH = 4000;
    ...
    要求：
    - 输出的是“当前分支状态层”，用于叠加在原著稳定档案之后；优先记录近期对话已经明确造成的变化，不要机械复述未变化的原著资料
    ...
    - personality 应写“当前表现出的决策倾向或行为规律＋证据”，不得只堆形容词；relations 应写关系对象、当前关系、变化原因和未解决张力
    - 严格控制字数，按下面结构输出，不输出任何其他文字：
    {"target":"${c.name}","appeared":true,"identity":"当前身份、目标与处境60字内","appearance":"持续有效的外貌或状态40字内或空字符串","personality":"当前决策倾向与行为证据80字内","relations":"当前重要关系、变化依据与张力120字内或空字符串"}
