...
export function niBuildDeviationRangeBatches(range, batchSize, totalFloor) {
    const normalized = niNormalizeDeviationFloorRange(
        range?.startFloor ?? range?.start ?? range?.from,
        range?.endFloor ?? range?.end ?? range?.to,
    );
    const total = Number(totalFloor);
    const size = Number(batchSize);
    if (!normalized || normalized.end < 1 || !Number.isInteger(total) || total < 1
        || !Number.isInteger(size) || size < 1) return [];
    const start = Math.max(1, normalized.start);
    const end = Math.min(total, normalized.end);
    if (start > end) return [];
    const batches = [];
    for (let cursor = start; cursor <= end; cursor += size) {
        const batchEnd = Math.min(end, cursor + size - 1);
        batches.push({ startFloor: cursor, endFloor: batchEnd, count: batchEnd - cursor + 1 });
    }
    return batches;
}

export function niPrepareDeviationResummarySections(existing, next) {
    const current = existing && typeof existing === 'object' ? existing : {};
    const incoming = next && typeof next === 'object' ? next : {};
    return {
        ...incoming,
        removedFacts: [],
        currentConstraint: String(current.currentConstraint ?? current.devCurrentConstraint ?? ''),
        preservedFacts: String(current.preservedFacts ?? current.devPreservedFacts ?? ''),
    };
}
...
export function createGenerationController(deps = {}) {
    ...
    let _niDevRangeBatchRunning = false;
    ...
    function niSetDevProgress(range, { advanceCovered = true } = {}) {
    ...
    function niSetDevButtonState({ running = false } = {}) {
        const active = !!running || _niDevRangeBatchRunning;
        ...
        if (btn) {
            ...
            icon.className = active ? 'ti ti-loader' : 'ti ti-analyze';
            icon.setAttribute('aria-hidden', 'true');
            btn.replaceChildren(icon, document.createTextNode(active ? '分析中…' : niDevButtonLabel()));
        }
        const rerunStart = q('#ni-dev-rerun-start');
        const rerunEnd = q('#ni-dev-rerun-end');
        const rerunButton = q('#ni-dev-rerun-btn');
        if (rerunStart) rerunStart.disabled = active;
        if (rerunEnd) rerunEnd.disabled = active;
        if (rerunButton) {
            rerunButton.disabled = active;
            rerunButton.setAttribute('aria-busy', active ? 'true' : 'false');
        }
        niSetDevRetryButtonState({ running: active });
    }
    ...
    function niBuildDeviationPrompt(promptTemplate, reference, recentMsgs, existingDeviation, rangeCtx = {}, mode = 'append', existingFacts = []) {
        ...
        const modeLine = mode === 'resummarize'
            ? '这是对指定历史楼层的补档重析。必须重新阅读本次范围，并把其中明确建立、可能因手动清理而从现有档案缺失的耐久事实重新写入 changed_facts，不能因为现有档案没有它就跳过。只补录已经发生的事件、长期关系变化、重要物品流转、永久伤亡、身份揭示、承诺与世界规则等耐久事实；不得把该历史范围内的旧地点、旧在场人物、临时动作、短期情绪或当时目标重新写成当前状态，也不得覆盖档案中来源更晚的状态。removed_facts 必须为空；current_deviation_constraint 与 preserved_facts 原样保留，程序不会用历史范围改写当前场景。'
            : mode === 'retry'
        ...
        if (mode === 'resummarize') {
            prompt += `\n\n【补档重析最终覆盖】\n本模式只恢复历史范围中缺失的耐久事实。上述关于改写 current_deviation_constraint、preserved_facts 或移除旧事实的通用要求在本模式不适用：removed_facts 必须为 []；current_deviation_constraint 与 preserved_facts 可以原样照抄现有值，程序也会强制保留现值。不要输出旧位置、旧在场状态、当时正在进行的动作、临时目标或短期情绪。`;
        }
    ...
    async function niRunDev(options = {}) {
        ...
        const requestedRange = niNormalizeDevRange(options.range);
        const resummarize = !!options.resummarize;
        const rangeBatch = !!options.rangeBatch;
        if (resummarize && !requestedRange) {
            const noteEl = q('#ni-dev-note');
            if (noteEl) noteEl.textContent = '指定楼层范围无效。';
            return { ok: false, reason: 'invalid_range' };
        }
        if (S.devRunning || (_niDevRangeBatchRunning && !rangeBatch)) {
            const noteEl = q('#ni-dev-note');
        ...
        const noteEl = q('#ni-dev-note');
        if (noteEl) {
            noteEl.textContent = resummarize
                ? `正在重新总结${niDevRangeLabel(requestedRange)}...`
                : (retry ? '正在重试偏差分析...' : '正在更新当前偏差...');
        }
        ...
        try {
            ...
            const targetRange = resummarize ? requestedRange : retryRange;
            const chatCtx = niBuildChatRangeContext(recentLimit, targetRange);
            const recentMsgs = chatCtx.text;
            if (noteEl && chatCtx.count) {
                noteEl.textContent = niDevRangeProgressLabel(chatCtx, resummarize ? '重新总结' : (retry ? '重试' : '更新'));
            }
            ...
            const prompt = niBuildDeviationPrompt(
                ...
                existingDeviation,
                chatCtx,
                resummarize ? 'resummarize' : (retry ? 'retry' : 'append'),
            ...
            let nextSections = niBuildDeviationSectionsFromAnalysis(json);
            if (resummarize) {
                nextSections = niPrepareDeviationResummarySections(existingSections, nextSections);
            }
            niSetDeviationSections(niMergeDeviationSections(existingSections, nextSections, {
                floor: chatCtx.endFloor,
                range: chatCtx,
            }));
            niSetDevProgress(chatCtx, { advanceCovered: !resummarize });
            niSyncDeviationResultUI({ collapsed: true });
            await niQueueDeviationGuideSave({ immediate: true });
            if (noteEl && resummarize) noteEl.textContent = `${niDevRangeLabel(chatCtx)}重新总结完成。`;
            return { ok: true, auto, retry, resummarize, recentLimit, range: chatCtx, coveredFloor: S.devCoveredFloor };
        } catch (e) {
    ...
    async function niRunDevRange(range) {
        const normalized = niNormalizeDevRange(range);
        const total = niCurrentChatFloorCount();
        const batchSize = niDevRecentMessageLimit(false);
        const batches = niBuildDeviationRangeBatches(normalized, batchSize, total);
        if (!normalized || !batches.length) return { ok: false, reason: 'invalid_range', results: [] };
        if (S.devRunning || _niDevAutoBatchRunning || _niDevRangeBatchRunning) {
            return { ok: false, skipped: true, reason: 'busy', results: [] };
        }

        _niDevRangeBatchRunning = true;
        niSetDevButtonState({ running: true });
        const results = [];
        try {
            for (let index = 0; index < batches.length; index++) {
                ...
                const noteEl = q('#ni-dev-note');
                if (noteEl) {
                    noteEl.textContent = `正在重新总结${niDevRangeLabel(batch)}（第 ${index + 1}/${batches.length} 批）...`;
                }
                const result = await niRunDev({
                    resummarize: true,
                    range: batch,
                    rangeBatch: true,
                    skipStateLoad: index > 0,
                });
                if (!result?.ok) return { ...result, results, failedRange: batch };
                results.push(result);
            }
            const completedRange = {
                startFloor: batches[0].startFloor,
                endFloor: batches[batches.length - 1].endFloor,
            };
            const noteEl = q('#ni-dev-note');
            if (noteEl) noteEl.textContent = `${niDevRangeLabel(completedRange)}重新总结完成，共 ${batches.length} 批。`;
            return {
                ok: true,
                resummarize: true,
    ...
    async function niMaybeAutoRunDev({ requireNewMessage = false, forceStart = false } = {}) {
        ...
        if (S.devRunning || _niDevAutoBatchRunning || _niDevRangeBatchRunning) {
            return { ok: false, skipped: true, reason: 'busy', floor, covered, every };
        }
