...
export function createGenerationController(deps = {}) {
    ...
    function niDevSetFactPager(pageInfo, { editing = false } = {}) {
        ...
        if (older) older.disabled = editing || !pageInfo.hasOlder;
        if (newer) newer.disabled = editing || !pageInfo.hasNewer;
        if (count) {
    ...
    function niRenderDeviationFactNotebook() {
        ...
        if (list) {
            ...
            niDevSetFactPager(pageInfo, { editing });
    ...
    async function niSaveDeviationChatState({ saveChat = true, chatRoot = null, syncFromUI = true } = {}) {
        let root = null;
    ...
    function niLoadDeviationStateFromChat({ allowLegacyMigration = false, collapsed = true, syncUI = true } = {}) {
        ...
        if (saved) {
            ...
            if ((needsSourceFloorMigration || needsStorageCompaction) && chatRoot) {
                // 旧聊天状态只在首次加载时回写一次：修复楼层元数据，并去掉重复保存的巨型文本副本。
