  ...
  "characters": [
    {
      ...
      "role": "主角|配角|反派|其他",
      "identity": "本段新增或修正的身份信息：出身、年龄、职位/封号、阵营、公开身份或已明确发生的身份变化；没有新证据则留空字符串",
      "appearance": "本段新增的稳定外貌与辨识特征；临时衣着、表情和伤势不写，没有新证据则留空字符串",
      "gender": "性别：原文有明确描写则照录（男/女/不明/其他），无任何描写则留空字符串",
      "personality": "本段提供新证据的稳定性格/决策规律，格式：'特征或行为规律：触发条件→具体行为依据'；临时情绪不写，没有新证据则留空字符串",
      "relations": "本段明确成立或改变的重要关系，格式：'角色名：关系性质（变化依据）'；多个用分号分隔，没有新证据则留空字符串"
    }
  ],
  "character_aliases": [
    {
...
【七、字段填写规则】
▌characters 录入规则：
- 收录本段首次登场的角色，以及本段再次出现且获得了新身份、稳定外貌、性格行为证据或关系变化的既有角色
- 对既有角色只填写本段新增、修正或能够提供新证据的字段；不要复述前文已有资料，没有新内容的字段留空字符串
- 性格必须由本段具体行为或明确旁白支持，禁止只贴“温柔、冷静、善良、腹黑”等无证据标签；临时情绪、一次性反应不得上升为稳定性格
- 关系字段既可记录初始关系，也可记录已经发生的关键变化，但必须写出变化依据；禁止把角色单方面声明直接当作双方关系事实
...
请严格按 JSON 输出，不要输出 Markdown、代码块或结构外文字：
{
  ...
  "current_deviation_constraint": "严格按【不可回滚约束】【当前场景约束】【已失效的原著前提】三段输出的连续性防火墙，不超过700字"
}
...
6. 对尚未明确成立、仍可自然变化的偏差，irreversible 为 false；不得为了贴合原著主动建议回收剧情，只能基于当前分支自然发展。
7. changed_facts 采用增量语义：只输出本批新增、被本批正文更新、或与本批直接相关而确有必要重申的事实。未出现在提示词或输出中的旧事实由程序在本地保留，不得为了凑完整快照而复述庞大旧账本。
8. 只有当前正文提供明确新证据，足以修正、替代或使旧事实失效时，才把旧事实 id 或原文写入 removed_facts；不要仅因旧事实未被本批再次提到就删除。
9. current_deviation_constraint 是本次更新后的完整连续性防火墙，不要只写本次增补。必须严格使用以下三段标题：\n【不可回滚约束】记录死亡、永久状态、身份暴露、不可逆关系/阵营/规则改变以及明确禁止恢复的旧前提；\n【当前场景约束】只记录当前地点、在场人物、正在执行的动作、临时目标与短期状态，并为每项写“有效至：场景变化/事项解决/被新状态覆盖”；\n【已失效的原著前提】只记录已被当前分支明确推翻、不能再作为后续前提的原著内容。没有内容的段落也保留标题并写“- 无”。
10. preserved_facts 是本次更新后仍然有效的原著事实完整列表，只保留尚未发生、未过期、未被当前正文推翻、且不会误导后续写作的原著逻辑。
11. current_deviation_constraint 应该是给后续写作模型看的，不要分析打分，不要解释 JSON，只写可验证的连续性约束；不得把一次动作解释成永久心理、所有权、爱恨或人格，除非正文明确确认。
12. current_deviation_constraint 必须明确：当前分支事实优先；已经结束的历史事件只是因果背景，不得当作尚未完成的剧情重新续接；后续基于最新状态自然推演，不得强行复刻原著事件。
13. changed_facts 与 current_deviation_constraint 必须分层：死亡、身份暴露、关系断裂、阵营改变、能力变化、已完成的关键行动等客观长期事实放入 changed_facts；<user> 或角色的当前所在地、同行者、临时目标、正在执行的动作、短期状态与暂时处境只写进【当前场景约束】，不得进入【不可回滚约束】。
14. changed_facts 只写客观事实，不要写“不能继续作为剧情前提”“后续不得……”等执行说明；这些内容只写进 current_deviation_constraint。
15. 如果相关旧事实中已有“<user> 当前在 A 地/正在做 A 事/与某人同行”等会随剧情推进过期的当前状态，而本次正文明确移动到 B 地或进入新处境，应输出最新状态，并把已被替代的旧状态列入 removed_facts；以最新状态为准。
16. 已有偏差档案中的长期硬事实，除非当前正文明确推翻、改写或自然修正，否则一律视为仍在本地保留；无需在 changed_facts 中机械复述。
17. 死亡、身份暴露、关系断裂、阵营改变、能力变化、已完成的关键行动等硬事实，即使最近正文没有再次提到，也必须默认仍有效；地点只在表示长期归属、封锁、流放、不可逆迁移等稳定结果时才作为长期锚点。
...
22. changed_facts 中本次新记录或正文被改写的事实必须填写 source_floor；它必须取自 <current> 中标出的【第 X 楼】，并选择最早明确成立该事实的实际对话楼层。不得使用自动总结所在楼层、偏差分析触发楼层、本次分析范围结束楼层或推测楼层。确有必要重申的旧事实不要改写其来源楼层。changed_facts 数组按 source_floor 从小到大排列。
23. changed_facts 的 importance 为 1-5：普通分支细节1、持续状态2、重要关系/秘密/承诺3、重大转折4、死亡、永久失能、不可撤销身份暴露或世界规则改写5。irreversible 只有在该事实已经不能自然撤回时才为 true；不得把临时地点、情绪和进行中的动作标成不可逆。
...
- current_deviation_constraint 是否不超过700字，并完整包含三个规定标题。
...
export const WORLD_EXTRACT_PROMPT = `你是专业的小说世界观资料编辑。你的任务是从有来源的剧情节点中整理可供后续角色扮演稳定执行的世界事实，而不是创作一个听起来合理的新世界。
以下是一部小说某一批次的剧情节点资料：
<nodes>
{NODES}
</nodes>

请提取与「{CATEGORY}」相关、在上述资料中有明确依据的世界设定。

输出要求：
- 每条单独一行，格式固定为：设定｜适用范围｜限制、例外或不确定性｜依据节点
- “依据节点”填写输入中的节点标题；同一事实有多个依据时可并列
- 区分世界硬规则、社会惯例、组织制度、角色传闻与阶段性状态；传闻或认知不确定时必须明确标注，不得当作客观硬规则
- 保留规则的代价、触发条件、边界和例外；不得为了简短删掉会影响剧情推演的限制
- 合并真正重复的事实，但不要把不同地区、时期、群体或层级的规则强行概括为一句
- 禁止补写资料中没有出现的常识、因果、地理、制度或能力规则
- 不输出分类标题、序号、markdown 或分析过程，直接输出设定行
- 若信息不足，输出「暂无相关设定」
- 单批最多输出8条，建议总字数控制在800字以内`;

// 世界设定默认大类配置
export const WORLD_SHRINK_PROMPT = `你是小说世界观资料库的合并编辑。以下内容来自同一世界设定分类的多个剧情批次，请合并为可直接注入后续角色扮演的最终资料：
- 每条单独一行，保持“设定｜适用范围｜限制、例外或不确定性｜依据节点”格式
- 合并重复项，保留不同地点、时期、群体、层级和阶段的差异
- 出现表面冲突时不要擅自裁决；若可由适用范围或时间变化解释，则明确范围，否则标注“资料存在冲突”并保留双方依据
- 删除只描述单次人物行为、却不能推出稳定规则的内容
- 不得新增输入中没有的事实，不得删掉代价、触发条件、知识边界和重要例外
- 最多保留12条，建议总字数控制在1200字以内
- 直接输出设定行，不加标题、序号、Markdown 或分析过程

{CONTENT}`;

export const WORLD_DEFAULT_CATEGORIES = [
    { id: 'boundary',  label: '基础规律与世界边界', enabled: true,  hint: '科技水平、自然规律、超自然现象，以及明确不存在或无法做到的事' },
    { id: 'mechanism', label: '能力体系与特殊机制', enabled: true,  hint: '修炼、异能、系统、特殊物质的来源、代价、限制与例外' },
    { id: 'geography', label: '地理地点与交通',     enabled: true,  hint: '地区、重要地点、距离、交通方式、环境差异与地点限制' },
    { id: 'factions',  label: '组织势力与权力结构', enabled: true,  hint: '国家、家族、组织、阵营、职位体系及其权力关系' },
    { id: 'society',   label: '制度阶层与社会规则', enabled: true,  hint: '法律、身份等级、婚姻继承、教育职业及社会运行规则' },
    { id: 'economy',   label: '经济资源与日常生活', enabled: false, hint: '货币、资源、产业、物价、生活条件与稀缺性' },
    { id: 'culture',   label: '文化习俗与价值禁忌', enabled: false, hint: '礼仪、宗教、称谓、风俗、群体价值与禁忌' },
    { id: 'history',   label: '历史事件与时代变化', enabled: false, hint: '影响当前世界的历史事件、制度变迁、战争与年代分期' },
];
