...
export function niMemoryNormalizeChatMessages(messages) {
    return (Array.isArray(messages) ? messages : [])
        .map((message, index) => {
            ...
            if (!niMemoryIsStoryMessage(message) || !text) return null;
    ...
    function renderStatus(override = '') {
        ...
        if (list) {
            ...
            list.innerHTML = visible.length
                ? visible.map(leaf => `<div class="ni-memory-item"><span>第 ${leaf.startFloor}-${leaf.endFloor} 楼</span><p>${memoryEscapeHtml(leaf.summary)}</p></div>`).join('') + controls
