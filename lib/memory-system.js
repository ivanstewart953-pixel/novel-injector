...
export function niMemoryNormalizeChatMessages(messages) {
    return (Array.isArray(messages) ? messages : [])
        .map((message, index) => {
            ...
            const system = message?.is_system === true || message?.extra?.isSmallSys === true || role === 'system';
...
export function niMemoryBuildNextBatch(messages, store, batchSize = 10, { force = false } = {}) {
    const normalized = niMemoryNormalizeChatMessages(messages);
    const current = niMemoryNormalizeStore(store);
    const size = Math.max(2, Math.min(50, parseInt(batchSize, 10) || 10));
    const pending = normalized.filter(entry => entry.floor > current.processedThrough);
    if (!pending.length || (!force && pending.length < size)) return null;
    const entries = pending.slice(0, size);
    return {
        entries,
...
export function niMemoryValidateExtractPayload(payload, batch) {
    ...
    if (!memoryText(payload.summary, 1600)) throw new Error('事实账本缺少 summary');
...
function memoryNormalizeEvent(event, startFloor, endFloor, index) {
    const text = memoryText(event?.text ?? event?.summary, 700);
    if (!text) return null;
...
export function niMemoryNormalizeLeafPayload(payload, batch) {
    ...
    const openThreads = (Array.isArray(payload?.open_threads) ? payload.open_threads : [])
        ...
        .filter(Boolean);
    const summary = memoryText(payload?.summary, 1600)
        || memoryText(events.map(event => event.text).join('；'), 1600)
...
export function niMemoryInvalidateChangedRanges(value, messages) {
    ...
    for (const leaf of store.leaves) {
        const entries = normalized.filter(entry => entry.floor >= leaf.startFloor && entry.floor <= leaf.endFloor);
        if (!entries.length || memoryBatchHash(entries) !== leaf.sourceHash) {
...
export function niMemoryBuildCompactionRecords(records) {
    return (Array.isArray(records) ? records : []).map(record => ({
        ...
        events: record.level === 'leaf'
            ? record.events?.map(event => ({ text: event.text, certainty: event.certainty, importance: event.importance }))
            : record.keyEvents,
...
function memoryDocumentText(document) {
    return [
        ...
        ...(document.events || []).flatMap(event => [event.text, ...(event.actors || []), ...(event.locations || []), ...(event.objects || []), ...(event.tags || [])]),
    ].filter(Boolean).join(' ');
}
...
export function niMemoryScoreDocuments(query, documents, { latestFloor = 0 } = {}) {
    ...
    return prepared.map(item => {
        ...
        const score = (bm25 + entityHits * 1.8 + importance * 0.8 + recency * 0.45 + fallback) * levelFactor;
...
export function niMemoryRecall(value, query, { topK = 5 } = {}) {
    const store = niMemoryNormalizeStore(value);
    ...
    const limit = Math.max(1, Math.min(12, parseInt(topK, 10) || 5));
    const selected = [];
...
function memoryDocumentLine(item) {
    ...
    const uncertain = item.level === 'leaf' && item.events?.length && item.events.every(event => event.certainty === 'uncertain')
        ? '【整体未确认】'
        : '';
    return `- 【第${item.startFloor}-${item.endFloor}楼·${level}】${uncertain}${item.summary}`;
}
...
export function niMemoryBuildInjection(value, query, { topK = 5, tokenBudget = 2800 } = {}) {
    const store = niMemoryNormalizeStore(value);
    if (!store.leaves.length) return '';
    const budget = Math.max(400, Math.min(12000, parseInt(tokenBudget, 10) || 2800));
    const header = [
        '[当前分支记忆·最高事实优先级]',
    ...
    takeSection('当前状态', states, Math.max(120, remaining * 0.38));

    const knowledge = memoryRankValues(Object.values(store.knowledge), query).map(memoryKnowledgeLine);
    takeSection('角色知识边界', knowledge, Math.max(80, remaining * 0.22));

    const threads = memoryRankValues(Object.values(store.openThreads), query).map(memoryThreadLine);
    takeSection('未解决事项', threads, Math.max(80, remaining * 0.30));

    const recalled = niMemoryRecall(store, query, { topK }).map(memoryDocumentLine);
    takeSection('本轮相关过往', recalled, remaining);
...
export function createBranchMemoryController(deps = {}) {
    ...
    async function captureOne({ force = false, compact = true } = {}) {
        ...
        try {
            ...
            renderStatus(`正在总结第 ${batch.startFloor}-${batch.endFloor} 楼…`);
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    const retryRule = attempt
                        ? '\n\n上一次输出未通过插件校验。请重新检查 JSON 是否完整、四个数组字段是否存在、每个数组项是否填写本批范围内的 evidence_floors；仍然只输出 JSON。'
                        : '';
    ...
    function renderStatus(override = '') {
        ...
        if (list) {
            const latest = store.leaves.slice(-4).reverse();
            list.innerHTML = latest.length
                ? latest.map(leaf => `<div class="ni-memory-item"><span>第 ${leaf.startFloor}-${leaf.endFloor} 楼</span><p>${memoryEscapeHtml(leaf.summary)}</p></div>`).join('')
    ...
    function saveUi() {
        ...
        cfg.branchMemoryBatchSize = memoryNumber(query('#ni-memory-batch-size')?.value, defaultSettings.branchMemoryBatchSize || 10, 2, 50);
        cfg.branchMemoryTopK = memoryNumber(query('#ni-memory-top-k')?.value, defaultSettings.branchMemoryTopK || 5, 1, 12);
        cfg.branchMemoryTokenBudget = memoryNumber(query('#ni-memory-token-budget')?.value, defaultSettings.branchMemoryTokenBudget || 2800, 400, 12000);
    ...
    function bindUi(root = globalThis.document) {
        root?.addEventListener?.('click', event => {
            const target = event.target?.closest?.('#ni-memory-update, #ni-memory-reset, #ni-memory-cfg-btn, #ni-memory-prompt-btn, #ni-memory-prompt-reset');
            if (!target) return;
        ...
        root?.addEventListener?.('change', event => {
            if (!event.target?.matches?.('#ni-memory-enabled, #ni-memory-batch-size, #ni-memory-top-k, #ni-memory-token-budget, #ni-memory-original-token-budget, #ni-memory-recent-count, #ni-memory-inj-pos, #ni-memory-inj-depth, #ni-memory-inj-role')) return;
            saveUi();
